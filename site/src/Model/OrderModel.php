...
class OrderModel extends ItemModel
{
	...
	{
		$app  = Factory::getApplication('com_galleries');
		$user = $app->getIdentity();

		// Check published state
		if ((!$user->authorise('core.edit.state', 'com_galleries')) && (!$user->authorise('core.edit', 'com_galleries')))
		{
		...
		{
			$id = Factory::getApplication()->getUserState('com_galleries.edit.order.id');
		}
		else
		{
			$id = Factory::getApplication()->input->get('id');
			Factory::getApplication()->setUserState('com_galleries.edit.order.id', $id);
		}
	...
	public function getItem($id = null)
	{
		if ($this->_item === null)
		{
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_GALLERIES_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_GALLERIES_ITEM_NOT_LOADED'), 404);
			}
		}
		...
		if (isset($this->_item->gallery_id) && $this->_item->gallery_id != '')
		{
			if (is_object($this->_item->gallery_id))
			{
				$this->_item->gallery_id = ArrayHelper::fromObject($this->_item->gallery_id);
			}

			$values = (is_array($this->_item->gallery_id)) ? $this->_item->gallery_id : explode(',',$this->_item->gallery_id);
			...
			$this->_item->gallery_id = !empty($textValue) ? implode(', ', $textValue) : $this->_item->gallery_id;
	...
	public function getItemIdByAlias($alias)
	{
		...
		{
			$aliasKey   = $this->getAliasFieldNameByView('order');
		}
